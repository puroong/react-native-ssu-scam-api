...
function generateSchduleExportForm($: cheerio.Root, option: ScheduleExportOption): ScheduleExportForm {
  ...
  const QF_CORE_CALENDAR_EXPORT_FORM = 1;

  if (sesskey === undefined) throw new Error('sesskey not found');

  return {
    sesskey,
    _qf__core_calendar_export_form: QF_CORE_CALENDAR_EXPORT_FORM,
...
export default function getSchedules(cookie: Cookie, option: ScheduleExportOption): Promise<CalendarResponse> {
  return axios
    .get(GET_SCHEDULES_URL, { headers: { Cookie: cookie.toString() } })
    .then((res) => {
      const $ = load(res.data);

      return generateSchduleExportForm($, option);
    })
    .then((schduleExportForm: ScheduleExportForm) =>
      axios.post(GET_SCHEDULES_URL, stringify(schduleExportForm), { headers: { Cookie: cookie.toString() } }),
    )
